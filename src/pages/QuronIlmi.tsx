...
import { quranIlmAPi, type QuranIlmType } from "../apis/quran-ilm.api";
...
const columns: Column<QuranIlmType>[] = [
...
const QuranIlm = () => {
  const [ilmlar, setIlmlar] = useState<QuranIlmType[]>([]);
  ...
  const [selectedIlm, setSelectedIlm] = useState<QuranIlmType | null>(null);
  const [isEditMode, setIsEditMode] = useState(false);
  const [deleteLoading, setDeleteLoading] = useState(false);
  const [fileData, setFileData] = useState<FileData | null>(null);
  const PAGE_SIZE = 10;
  const { create, remove, update, getPaginated } = quranIlmAPi;
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Qur'an ilmlarini yuklashda xatolik yuz berdi"
      );
      setIlmlar([]);
      setTotalPages(1);
    } finally {
  ...
  const handleView = async (row: QuranIlmType) => {
    setSelectedIlm(row);
  ...
  const handleEdit = (row: QuranIlmType) => {
    setSelectedIlm(row);
    setIsEditMode(true);
  ...
  const handleDelete = (row: QuranIlmType) => {
    setSelectedIlm(row);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success("Qur'an ilmi muvaffaqiyatli o'chirildi");
      setCurrentPage(1); // Reset to first page after deletion
      setConfirmModalOpen(false);
    } catch (err: any) {
      toast.error(
        err.response?.data?.message ||
          "Qur'an ilmini o'chirishda xatolik yuz berdi"
      );
    } finally {
  ...
  const handleFormSubmit = async (data: Record<string, any>) => {
    try {
      if (isEditMode && selectedIlm) {
        ...
        toast.success("Qur'an ilmi muvaffaqiyatli yangilandi");
      } else {
        await create(data as any);
        toast.success("Qur'an ilmi muvaffaqiyatli yaratildi");
      ...
      toast.error(
        err.response?.data?.message ||
          "Qur'an ilmini saqlashda xatolik yuz berdi"
      );
      throw err;
    }
  };
  ...
  const getViewFields = (ilm: QuranIlmType): ViewField[] => {
    ...
    fields.push(
      ...
      },
      {
        ...
        value: dateFormatted(ilm.created_at),
      },
      {
        label: "Yangilangan vaqti",
        value: dateFormatted(ilm.updated_at),
      }
    );
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-xl font-semibold">Qur'an Ilmlari</h1>
        <button
          onClick={handleAdd}
      ...
      {selectedIlm && (
        <ViewModal
          ...
          title={`Qur'an ilmini ko'rish: ${selectedIlm.title_en}`}
      ...
      {selectedIlm && (
        <ConfirmModal
          ...
          title="Qur'an ilmini o'chirish"
          message={`Bu Qur'an ilmi o'chirilmoqda: "${selectedIlm.title_en}"? O'chirishni tasdiqlang.`}
      ...
      <FormModal
        ...
        title={
          isEditMode ? "Qur'an ilmini yangilash" : "Yangi Qur'an ilmi qo'shish"
        }
        fields={formFields}
