...
const IlmBob = () => {
  ...
  const PAGE_SIZE = 10;
  const { create, remove, update, getPaginated } = ilmBobApi;
    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Ilm boblarini yuklashda xatolik yuz berdi"
      );
      setBoblar([]);
      setTotalPages(1);
    } finally {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success("Ilm bobi muvaffaqiyatli o'chirildi");
      ...
      toast.error(
        err.response?.data?.message ||
          "Ilm bobini o'chirishda xatolik yuz berdi"
      );
    } finally {
  ...
  const handleFormSubmit = async (data: Record<string, any>) => {
    try {
      if (isEditMode && selectedBob) {
        ...
        toast.success("Ilm bobi muvaffaqiyatli yangilandi");
      } else {
        await create(data as any);
        toast.success("Ilm bobi muvaffaqiyatli yaratildi");
      ...
      toast.error(
        err.response?.data?.message || "Ilm bobini saqlashda xatolik yuz berdi"
      );
      throw err;
    }
  };
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-xl font-semibold">Ilm Boblari</h1>
        <button
          onClick={handleAdd}
      ...
      {selectedBob && (
        <ViewModal
          ...
          title={`Ilm bobini ko'rish: ${selectedBob.title_en}`}
      ...
      {selectedBob && (
        <ConfirmModal
          ...
          title="Ilm bobini o'chirish"
          message={`Bu ilm bobi o'chirilmoqda: "${selectedBob.title_en}"? O'chirishni tasdiqlang.`}
      ...
      <FormModal
        ...
        title={isEditMode ? "Ilm bobini yangilash" : "Yangi ilm bobi qo'shish"}
        fields={formFields}
